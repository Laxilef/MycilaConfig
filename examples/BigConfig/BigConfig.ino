...
void setup() {
  ...
  config.listen([](const char* key, const WrappedConfig::Value& newValue) {
    if (newValue.isNull()) {
      Serial.printf("[CHANGE] %s = NULL\n", key);

    } else {
      Serial.printf("[CHANGE] %s = %s\n", key, newValue.as<const char*>());
      //Serial.printf("[CHANGE] %s = %s\n", key, newValue.as<std::string>().c_str());
    }
  });
...
void loop() {
  ...
  if (op < 40) {
    ...
    if (!result) {
      auto rnd = random(0, 1000);
      if (!config.set(key, (uint16_t)rnd) && !config.set(key, (float)rnd * 1.33f)) {
        // oops
      }
    }
  }
